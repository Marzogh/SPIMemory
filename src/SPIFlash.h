...
public:
    ...
    #else
    void
    setClock(uint8_t clockdiv);
    #endif
    bool
    libver(uint8_t * b1, uint8_t * b2, uint8_t * b3);
    bool
    sfdpPresent(void);
    uint8_t
    error(bool verbosity = false);
    uint16_t
    getManID(void);
    uint32_t
    getJEDECID(void);
    ...
    getUniqueID(void);
    uint32_t
    getAddress(uint16_t size);
    uint16_t
    sizeofStr(String &inputStr);
    uint32_t
    getCapacity(void);
    uint32_t
    getMaxPage(void);
    float
    functionRunTime(void);
    // -------------------------------- Write / Read Bytes ---------------------------------//
    bool
    ...
    bool
    writeByteArray(uint32_t _addr, uint8_t * data_buffer, size_t bufferSize, bool errorCheck = true);
    bool
    ...
    bool
    writeCharArray(uint32_t _addr, char * data_buffer, size_t bufferSize, bool errorCheck = true);
    bool
    ...
    writeAnything(uint32_t _addr, const T& data, bool errorCheck = true);
    template < class T > bool
    readAnything(uint32_t _addr, T& data, bool fastRead = false);
    // -------------------------------- Erase functions ------------------------------------//
    bool
    eraseSection(uint32_t _addr, uint32_t _sz);
    bool
    eraseSector(uint32_t _addr);
    bool
    eraseBlock32K(uint32_t _addr);
    bool
    eraseBlock64K(uint32_t _addr);
    bool
    eraseChip(void);
    // -------------------------------- Power functions ------------------------------------//
    bool
...
private:
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
    unsigned
    _createMask(unsigned a, unsigned b);
    void
    _setDefaultParams(void);
    void
    _troubleshoot(uint8_t _code, bool printoverride = false);
    void
    _endSPI(void);
    bool
    _disableGlobalBlockProtect(void);
    bool
    _isChipPoweredDown(void);
    bool
    _prep(uint8_t opcode, uint32_t _addr, uint32_t size = 0);
    bool
    _startSPIBus(void);
    bool
    _beginSPI(uint8_t opcode);
    bool
    _noSuspend(void);
    bool
    _notBusy(uint32_t timeout = BUSY_TIMEOUT);
    bool
    _notPrevWritten(uint32_t _addr, uint32_t size = 1);
    bool
    _writeEnable(bool _troubleshootEnable = true);
    bool
    _writeDisable(void);
    bool
    _transferAddress(void);
    bool
    _addressCheck(uint32_t _addr, uint32_t size = 1);
    bool
    _enable4ByteAddressing(void);
    bool
    _disable4ByteAddressing(void);
    uint8_t
    _nextByte(uint8_t data = NULLBYTE);
    uint16_t
    _nextInt(uint16_t = NULLINT);
    void
    _nextBuf(uint8_t opcode, uint8_t * data_buffer, uint32_t size);
    uint8_t
    _readStat1(void);
    uint8_t
    _readStat2(void);
    ...
    _readStat3(void);
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
    //    From SPIFlashSFDP.cpp    //
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
    bool
    _getSFDPTable(uint32_t _tableAddress, uint8_t * data_buffer, uint8_t numberOfDWords);
    bool
    _getSFDPData(uint32_t _address, uint8_t * data_buffer, uint8_t numberOfBytes);
    uint32_t
    _getSFDPdword(uint32_t _tableAddress, uint8_t dWordNumber);
    uint16_t
    _getSFDPint(uint32_t _tableAddress, uint8_t dWordNumber, uint8_t startByte);
    uint8_t
    _getSFDPbyte(uint32_t _tableAddress, uint8_t dWordNumber, uint8_t byteNumber);
    bool
    _getSFDPbit(uint32_t _tableAddress, uint8_t dWordNumber, uint8_t bitNumber);
    uint32_t
    _getSFDPTableAddr(uint32_t paramHeaderNum);
    uint32_t
    _calcSFDPEraseTimeUnits(uint8_t _unitBits);
    bool
    _checkForSFDP(void);
    void
    _getSFDPEraseParam(void);
    void
    _getSFDPProgramTimeParam(void);
    bool
    _getSFDPFlashParam(void);
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
    //     From SPIFlashID.cpp     //
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
    bool
    ...
    _idWinbond(void);
    bool
    _chipID(uint32_t flashChipSize = 0);
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
    //       From SPIFlash.h       //
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
    template < class T > bool
    _write(uint32_t _addr, const T& value, uint32_t _sz, bool errorCheck);
    template < class T > bool
    _read(uint32_t _addr, T& value, uint32_t _sz, bool fastRead = false);
    // template <class T> bool _writeErrorCheck(uint32_t _addr, const T& value);
    template < class T > bool
    _writeErrorCheck(uint32_t _addr, const T& value, uint32_t _sz);
    // -------------------------------- Private variables ----------------------------------//
    #ifdef SPI_HAS_TRANSACTION
...
template < class T > bool
SPIFlash::writeAnything(uint32_t _addr, const T& data, bool errorCheck)
{
...
template < class T > bool
SPIFlash::readAnything(uint32_t _addr, T& data, bool fastRead)
{
...
template < class T > bool
SPIFlash::_writeErrorCheck(uint32_t _addr, const T& value, uint32_t _sz)
{
...
template < class T > bool
SPIFlash::_write(uint32_t _addr, const T& value, uint32_t _sz, bool errorCheck)
{
    bool _retVal;
    ...
    if (!errorCheck) {
        ...
        // Serial.println(_addr);
        _retVal = _writeErrorCheck(_addr, value, _sz);
    }
    #ifdef RUNDIAGNOSTIC
    _spifuncruntime = micros() - _spifuncruntime;
    #endif
...
template < class T > bool
SPIFlash::_read(uint32_t _addr, T& value, uint32_t _sz, bool fastRead)
{
    #ifdef RUNDIAGNOSTIC
    _spifuncruntime = micros();
    #endif
