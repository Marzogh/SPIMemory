...
private:
    ...
    bool
    _prep(uint32_t _addr, uint32_t size = 0);
    bool
    _startSPIBus(void);
    bool
    ...
    uint8_t
    _nextByte(char IOType, uint8_t data = NULLBYTE);
    uint16_t
    ...
    _readStat1(void);
    template < class T > bool
    _write(uint32_t _addr, const T& value, uint32_t _sz, bool errorCheck, uint8_t _dataType);
    template < class T > bool
    _read(uint32_t _addr, T& value, uint32_t _sz, bool fastRead = false, uint8_t _dataType = 0x00);
    // template <class T> bool _writeErrorCheck(uint32_t _addr, const T& value);
    template < class T > bool
    _writeErrorCheck(uint32_t _addr, const T& value, uint32_t _sz, uint8_t _dataType = 0x00);
    // -------------------------------- Private variables ----------------------------------//
...
template < class T > bool
SPIFram::writeAnything(uint32_t _addr, const T& data, bool errorCheck)
{
...
template < class T > bool
SPIFram::readAnything(uint32_t _addr, T& data, bool fastRead)
{
...
template < class T > bool
SPIFram::_writeErrorCheck(uint32_t _addr, const T& value, uint32_t _sz, uint8_t _dataType)
{
...
template < class T > bool
SPIFram::_write(uint32_t _addr, const T& value, uint32_t _sz, bool errorCheck, uint8_t _dataType)
{
    bool _retVal;
...
template < class T > bool
SPIFram::_read(uint32_t _addr, T& value, uint32_t _sz, bool fastRead, uint8_t _dataType)
{
    #ifdef RUNDIAGNOSTIC
    _spifuncruntime = micros();
    #endif
