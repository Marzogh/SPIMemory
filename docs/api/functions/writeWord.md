...
- `bool SPIFlash::writeWord(uint32_t _addr, uint16_t data, bool errorCheck = true)`
- `bool SPIFram::writeWord(uint32_t _addr, uint16_t data, bool errorCheck = true)`

## What It Is For

Write one `uint16_t` value to memory at a specific address.
...
bool ok = flash.writeWord(addr, 65000);
