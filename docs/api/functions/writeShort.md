...
- `bool SPIFlash::writeShort(uint32_t _addr, int16_t data, bool errorCheck = true)`
- `bool SPIFram::writeShort(uint32_t _addr, int16_t data, bool errorCheck = true)`

## What It Is For

Write one `int16_t` value to memory at a specific address.
...
bool ok = flash.writeShort(addr, -1234);
