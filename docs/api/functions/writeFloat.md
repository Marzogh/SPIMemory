...
- `bool SPIFlash::writeFloat(uint32_t _addr, float data, bool errorCheck = true)`
- `bool SPIFram::writeFloat(uint32_t _addr, float data, bool errorCheck = true)`

## What It Is For

Write one `float` value to memory at a specific address.
...
bool ok = flash.writeFloat(addr, 3.14f);
