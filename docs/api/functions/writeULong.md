...
- `bool SPIFlash::writeULong(uint32_t _addr, uint32_t data, bool errorCheck = true)`
- `bool SPIFram::writeULong(uint32_t _addr, uint32_t data, bool errorCheck = true)`

## What It Is For

Write one `uint32_t` value to memory at a specific address.
...
bool ok = flash.writeULong(addr, 876532UL);
