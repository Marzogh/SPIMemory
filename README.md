...
###### `setClock(clockSpeed)`
Must be called right before begin(). If `SPI_HAS_TRANSACTION` is defined This function takes a 32-bit number as replacement for the default maximum clock speed (104MHz for Winbond NOR flash). Otherwise takes `SPI_CLOCK_DIVx`. In either case sets the clock speed for SPI. Use with care.
