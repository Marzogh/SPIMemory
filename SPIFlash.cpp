...
#ifndef HIGHSPEED
bool SPIFlash::_notPrevWritten(uint32_t address, uint16_t size) {
	uint32_t _size = size;
	uint32_t sampleSize;
	if (_size <= 10) {
		sampleSize = _size;
...
//Checks for and initiates the chip by requesting the Manufacturer ID which is returned as a 32 bit int
